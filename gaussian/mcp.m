...
if (~isfield(opt,'gamma')); gamma = 3; else gamma = opt.gamma; end
...
if (~isfield(opt,'lambda'));
    lambda_max = max((X'*(y.*weight))./v)/n;  %mcp/mcp
...
for nl = 1:nlambda
    ...
    while ~converge
        ...
        for j = 1:p
            ...
            beta_new(j) = mcp_filter(zj, lambda(nl), v(j), gamma);
